...
export default function SwipeCard({
  ...
  return (
    <motion.div
      className="absolute inset-0 select-none overflow-hidden rounded-2xl bg-neutral-900 shadow-xl"
      drag={active ? 'x' : false}
    ...
    >
      {title.posterUrl ? (
        ...
        <div className="flex h-2/3 w-full items-center justify-center bg-neutral-800 text-neutral-500">No poster</div>
      )}
      <div className="p-4">
        <h2 className="text-lg font-semibold">
          {title.title} {title.year ? `(${title.year})` : ''}
        </h2>
        <div className="mt-1 flex gap-3 text-sm text-neutral-400">
          {title.imdbRating != null && <span>★ {title.imdbRating.toFixed(1)}</span>}
          {title.runtimeMinutes != null && <span>{title.runtimeMinutes} min</span>}
        </div>
        <p className="mt-2 line-clamp-2 text-sm text-neutral-300">{title.overview}</p>
      </div>
    </motion.div>
  )
}
